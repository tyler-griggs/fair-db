...
public:
  DBClient(const int client_id, ReaderWriterQueue<DBRequest> *request_queue,
      ...
      : client_id(client_id), request_queue_(request_queue),
        db_size_elements(db_size_elements), read_size(read_size),
        num_reads(num_reads) {}

  std::thread RunSequential() {
    srand(time(0));
    cout << "Client ID " << client_id << " running Sequential." << endl;
    std::thread client_thread(RunSequentialInternal, client_id, request_queue_,
                              db_size_elements, read_size, num_reads);
  ...
  std::thread RunRandom() {
    ...
    std::thread client_thread(RunRandomInternal, client_id, request_queue_,
                              db_size_elements, read_size, num_reads);
    return client_thread;
  }
...
private:
  ...
  static void RunSequentialInternal(const int client_id,
                                    ReaderWriterQueue<DBRequest> *request_queue,
                                    const size_t db_size_elements,
                                    const size_t read_size,
                                    const size_t num_reads) {
  ...
  static void RunRandomInternal(const int client_id,
                                ReaderWriterQueue<DBRequest> *request_queue,
                                const size_t db_size_elements,
                                const size_t read_size,
                                const size_t num_reads) {
    size_t num_random = 16;
    for (int i = 0; i < num_reads; ++i) {
      std::this_thread::sleep_for(std::chrono::milliseconds(1)); // 1kqps
