...
//                   3) handle ops that were started by readers but not finished by computers
// real DRFQ
...
int main() {
  ...
  for (int i = 0; i < num_runs; ++i) {
    ...
    std::vector<ClientQueueState> to_cpu_queue_state{client0_cpu_queue, client1_cpu_queue};
    auto all_to_cpu_queue_state =
        std::make_shared<AllQueueState>(to_cpu_queue_state);
    std::vector<ClientQueueState> to_disk_queue_state{client0_disk_queue, client1_disk_queue};
    ...
    std::vector<DBWorkerOptions> worker_options{
        worker0_options, worker1_options};
    ...
    db_manager.Run(
        worker_options, num_worker_threads, worker_cores, run_duration_s, num_clients);
