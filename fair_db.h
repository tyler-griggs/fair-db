...
public:
  void Init(size_t db_size_elements) override {
    const std::string filepath("/mnt/disks/db/db.bin");
  ...
  void Read(long& dummy, size_t start, size_t len) override {
    ...
    for (size_t i = 0; i < len; i += CHUNK_SIZE) {
      ...
      // cout << "i=" << i << ", curChunkSize=" << currentChunkSize << ", bufferSize=" << read_buffer_.size() <<  endl;
      read_file_.read(reinterpret_cast<char*>(&read_buffer_[0]), currentChunkSize * sizeof(int));
